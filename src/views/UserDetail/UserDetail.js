...
class UserDetail extends React.Component {
  ...
  loadPatronBlocks() {
    ...
    if (!user || this.patronBlocksResolver) {
      return;
    }
    ...
    const manualPatronBlocksResolver = stripes.hasInterface('feesfines')
      ...
      ? hasManualPatronBlocks
        .GET({ path: `manualblocks?query=userId==${user.id}&limit=2000` }).catch(() => [])
      : Promise.resolve([]);
    const automatedPatronBlocksResolver = stripes.hasInterface('automated-patron-blocks')
      ...
      ? hasAutomatedPatronBlocks
        .GET({ path: `automated-patron-blocks/${user.id}` }).catch(() => [])
      : Promise.resolve([]);

    this.patronBlocksResolver = Promise.all([
